...
const API = (function () {
  ...
  return {
    ...
    add: function (title, description, price, location, imgName, categoryId, duration, userId, jwtToken, callback) {
      $.ajax({
        url: '/api/anounces',
        method: 'POST',
        headers: headers(jwtToken),
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({title, description, price, location, imgName, categoryId, duration, 'ownerId': userId})
      }).done(callback)
    },
